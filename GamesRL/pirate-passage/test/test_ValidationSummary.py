...
class Test_ValidationSummary(unittest.TestCase):
    def test_init_at_zero(self):
        self.assertEquals(0, ValidationSummary().severity_code)

    def test_update_severity(self):
        summ = ValidationSummary()
        self.assertEquals(1, summ._update_severity("warning"))
        self.assertEquals(2, summ._update_severity("error"))
        self.assertEquals(2, summ._update_severity("warning"))
    ...
    def add_failure_condition(self):
        ...
        self.assertEquals(summ.severity_code, 1)

        summ.add_failure_condition(cond)
        cond = ("error", "There was an error")
        self.assertEquals(summ.severity_code, 2)
